...
if (!pkg) {
  console.warn(
    `[fabric-llama-embeddings] No prebuilt binary available for ${key}. ` +
...
try {
  ...
  if (found) {
    console.log(`[fabric-llama-embeddings] Native binary found (${key}).`);
  } else {
    console.warn(
      `[fabric-llama-embeddings] Package ${pkg} installed but binary not found. ` +
        `Embedding generation may not work.`,
    );
  }
} catch {
  console.warn(
    `[fabric-llama-embeddings] Platform package ${pkg} not installed. ` +
