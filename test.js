/**
 * Basic tests for fabric-llama-embeddings.
 *
 * Run with a model file:
 *   MODEL_PATH=/path/to/model.gguf node --test test.js
 *
 * Without MODEL_PATH, only unit tests (normalization, token building) run.
 */

import { describe, it, before, after } from "node:test";
import assert from "node:assert/strict";
import { init, embed, dimensions, dispose } from "./index.js";
    ...
    try {
      ...
      assert.ok(
        err.message.includes("Model file not found") ||
          err.message.includes("No llama-addon.node binary found"),
...
describe("error handling", () => {
  ...
  it("throws for missing modelPath", async () => {
    await assert.rejects(() => init({}), /modelPath is required/);
  });

  it("throws for nonexistent model file", async () => {
    await assert.rejects(
      () => init({ modelPath: "/nonexistent.gguf" }),
...
describe("embedding generation", { skip: !MODEL_PATH }, () => {
  ...
  it("generates an embedding vector", async () => {
    ...
    assert.ok(
      ...
      `Expected unit vector, magnitude = ${mag}`,
    );
  });
